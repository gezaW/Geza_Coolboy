...
uses
  ...
  Datasnap.DSHTTP, ServerContainerUnit;
...
type
  TWebModule1 = class(TWebModule)
    ...
    procedure DSRESTWebDispatcher1ParseRequest(Sender: TObject; const ARequest: string;
      const ASegments: TStrings; var ADSMethodName: string; const AParamValues: TStrings);
    procedure WebModule1testAction(Sender: TObject; Request: TWebRequest; Response: TWebResponse;
      var Handled: Boolean);
  private
...
uses ServerMethodsUnit,  Web.WebReq,restFelixMainUnit,
ServerIntf;

procedure TWebModule1.DSRESTWebDispatcher1ParseRequest(Sender: TObject; const ARequest: string;
  const ASegments: TStrings; var ADSMethodName: string; const AParamValues: TStrings);
  var ares: string;
begin

  aRes := 'GezasTestwegenParam';
  ///
end;
...
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := ServerMethodsUnit.TAPI.ClassName
